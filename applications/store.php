...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    redirect(url('applications/create.php'));
}
...
if ($errors) {
    ...
    <div class="card border-0 shadow-sm"><div class="card-body p-4"><form method="post" action="<?= e(url('applications/store.php')) ?>" class="row g-3"><?php require __DIR__ . '/_form.php'; ?><div class="col-12"><button class="btn btn-primary" type="submit">Save application</button></div></form></div></div>
    <?php
...
$year = date('Y');
$s = $pdo->prepare('SELECT application_number FROM credit_applications WHERE application_number LIKE ? ORDER BY application_number DESC LIMIT 1');
$s->execute(['APP-' . $year . '-%']);
$last = $s->fetchColumn();
$next = $last ? ((int) substr($last, -4) + 1) : 1;
$applicationNumber = 'APP-' . $year . '-' . str_pad((string) $next, 4, '0', STR_PAD_LEFT);

$data = $application;
$data['application_number'] = $applicationNumber;
$data['requested_amount'] = (float) $data['requested_amount'];
$data['requested_term_months'] = (int) $data['requested_term_months'];
$data['interest_rate'] = $data['interest_rate'] === null ? null : round((float) $data['interest_rate'], 4);
$data['annual_debt_service_amount'] = calculate_annual_debt_service_amount($data['requested_amount'], $data['interest_rate'], $data['requested_term_months']);
$data['existing_exposure_amount'] = (float) $data['existing_exposure_amount'];
$data['proposed_total_exposure_amount'] = (float) $data['proposed_total_exposure_amount'];

$s = $pdo->prepare(
    'INSERT INTO credit_applications (client_id, application_number, application_date, requested_amount, currency, requested_term_months, interest_rate, annual_debt_service_amount, credit_product, credit_purpose, repayment_source, existing_exposure_amount, proposed_total_exposure_amount, status, priority, notes)
     VALUES (:client_id, :application_number, :application_date, :requested_amount, :currency, :requested_term_months, :interest_rate, :annual_debt_service_amount, :credit_product, :credit_purpose, :repayment_source, :existing_exposure_amount, :proposed_total_exposure_amount, :status, :priority, :notes)'
);
$s->execute($data);
$id = (int) $pdo->lastInsertId();
log_action($pdo, 'create application', 'application', $id, null, $data);
