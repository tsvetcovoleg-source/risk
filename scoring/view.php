...
<div class="row g-4 mb-4">
    <div class="col-lg-5"><div class="card border-0 shadow-sm h-100"><div class="card-body p-4"><h2 class="h5 mb-3">Application and client</h2>
        <?php foreach (['Application number' => $data['application_number'], 'Client name' => $data['client_name'], 'IDNO' => $data['idno'], 'Requested amount' => format_amount($data['requested_amount'], $data['currency']), 'Currency' => $data['currency'], 'Annual interest rate' => format_interest_rate($data['interest_rate'] ?? null), 'Estimated annual debt service' => (($data['annual_debt_service_amount'] ?? null) === null || ($data['annual_debt_service_amount'] ?? '') === '' ? 'N/A' : format_amount($data['annual_debt_service_amount'], $data['currency'])), 'Application status' => $data['status']] as $label => $value): ?><div class="mb-2"><span class="text-secondary small"><?= e($label) ?></span><div class="fw-semibold"><?= e($value ?: 'N/A') ?></div></div><?php endforeach; ?>
