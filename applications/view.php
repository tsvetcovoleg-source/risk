...
if ($id && $pdo instanceof PDO) {
    ...
    if ($application) {
        ...
        $statement = $pdo->prepare('SELECT * FROM financial_periods WHERE application_id = ? AND deleted_at IS NULL ORDER BY period_end_date DESC, id DESC');
        $statement->execute([$id]);
...
<div class="row g-3"><?php foreach (['Financial ratios', 'Collateral', 'Scoring', 'Credit memo', 'Committee decision'] as $module): ?><div class="col-md-4"><div class="card module-placeholder border-0 shadow-sm h-100"><div class="card-body"><h3 class="h6 mb-2"><?= e($module) ?></h3><p class="text-secondary mb-0">This module will be implemented in the next development stages.</p></div></div></div><?php endforeach; ?></div>
